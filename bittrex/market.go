...
func GetSummaryTask() {
	...
	for {
		...
		imincnt++
		if imincnt == 60 {
			SummaryHourReport(smap)
			imincnt = 0
		}

		if imincnt != 0 && imincnt%15 == 0 {
			Summary15MinReport(smap)
		}
...
func SummaryHourReport(smap map[string][]*MarketSummary) {

	Top5 := make([]SummaryReport, 5, 5)
	zero, _ := decimal.NewFromString("0.0")
	...
	for k, v := range smap {
		...
		for i := range Top5 {
			if rpt.Vchg.Sub(Top5[i].Vchg).GreaterThan(zero) {
	...
	for i := range Top5 {
		emailText += fmt.Sprintf("                  %s                   \n", Top5[i].MarketName)
		emailText += fmt.Sprintf("Price     : %s => %s (%%%s) \n", Top5[i].Pfrom.String(), Top5[i].Pto.String(), (Top5[i].Pchg.Div(Top5[i].Pfrom)).Mul(percent).String())
		emailText += fmt.Sprintf("BaseVolume: %s => %s (VolumeChange %s, %%%s) \n", Top5[i].Vfrom.String(), Top5[i].Vto.String(), Top5[i].Vchg.String(),
			Top5[i].Vchg.Div(Top5[i].Vfrom).Mul(percent).String())
		emailText += fmt.Sprintf("\n")

		if i == 4 {
			Topic += Top5[i].MarketName
		} else {
			Topic += (Top5[i].MarketName + ",")
		}
	}
...
func Summary15MinReport(smap map[string][]*MarketSummary) {
